...
<script>
	//이전 버튼 이벤트
	function fn_prev(page, range, rangeSize, search){
		var page = parseInt(page) - 1;
		var range = Math.ceil(page/5);
		
		var url = "${contextPath}/board/search.do?search="+search;
		url = url + "&page=" + page;
		url = url + "&range=" + range;
	...
	//페이지 번호 클릭
	function fn_paging(page, range, rangeSize, search){
		var url = "${contextPath}/board/search.do?search="+search;
		url = url + "&page=" + page;
		url = url + "&range=" + range;
	...
	//다음 버튼 이벤트
	function fn_next(page, range, rangeSize, search){
		var page = parseInt(page) + 1;
		if(page % 5 == 1){
	...
	<div class = "pagingBox">
		<div class = "prev">
			<c:choose>
				<c:when test="${paging.prev }">
					<a onclick = "fn_prev('${paging.page }', '${paging.range }', '${paging.rangeSize }', '${search }')">이전</a>
				</c:when>
				<c:otherwise>
		...
		<div class="page">
			<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="idx">
				<a onclick="fn_paging('${idx}', '${paging.range}', '${paging.rangeSize}', '${search }')"> ${idx} </a>
			</c:forEach>
		</div>
		...
		<div class="next">
			<c:choose>
				<c:when test="${paging.next }">
					<a onclick="fn_next('${paging.page}', '${paging.range }', '${paging.rangeSize }', '${search }')">다음</a>
				</c:when>
				<c:otherwise>
					<a href="#" style="text-decoration: none; cursor: default">　　</a>
				</c:otherwise>
